...
tl = tiledlayout(h,2,1,'TileSpacing','compact');
tl.Title.String = tit;
tl.Title.FontName = 'JetBrainsMono';
...
ylabel('Trial Number');
title([' Fixed Value: ' num2str(ana.colorFixed)]);
...
try
	...
	title(sprintf('Threshold:%.2f Slope:%.2f Trials:%i',pv(1),pv(2),sum(totalVals)));
catch ME
	fprintf('===>>> Cannot plot psychometric curve yet...\n');
end
box on; grid on;
tl.XLabel.String = 'Variable Color Value';
